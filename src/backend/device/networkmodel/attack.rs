...
use crate::backend::device::{ConnectionGraph, Device, DeviceId, IdToDeviceMap};
...
pub fn enqueue_malicious_messages(
    ...
    for (frequency, malicious_message) in malicious_messages {
        ...
        message_queue.add_message(*frequency, *malicious_message);
...
pub fn process_malware(
    ...
    for device in device_map.devices() {
        ...
        message_queue.add_message(WIFI_2_4GHZ_FREQUENCY, malicious_message);
    }
}
...
mod tests {
    ...
    use crate::backend::device::{
        ConnectionGraph, DeviceBuilder, DeviceId, IdToDeviceMap, PowerSystem,
        Topology, TRXSystem
    };
